...
void ARoundGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
	Super::InitGame(MapName, Options, ErrorMessage);

	FHttpRequestRef Request = FHttpModule::Get().CreateRequest();
	Request->OnProcessRequestComplete().BindUObject(this, &ARoundGameMode::OnResponseReceived);
	Request->SetURL("https://us-central1-endless-runner-rsn-1.cloudfunctions.net/roundStart");
	Request->SetVerb("GET");
	Request->ProcessRequest();
}

void ARoundGameMode::OnResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("Response ARoundGameMode: %s"), *Response->GetContentAsString());
...
void ARoundGameMode::StartSession()
{
	...
	GetWorld()->GetTimerManager().SetTimer(RespawnTimerHandle, this, &ARoundGameMode::OnRespawn, 0.1f, false);
}

void ARoundGameMode::InitializeGame()
{
	PreviousPlatform = nullptr;

	PlatformCount = 0;

	FVector Position = ReferenceToSpawn->GetSpawnOrigin()->GetActorLocation();
...
void ARoundGameMode::Tick(float DeltaTime)
{
	...
	if (ReferenceToSpawn != nullptr)
	{
		FVector FloorPosition = ReferenceToSpawn->GetFloorPlane()->GetActorLocation();
		FloorPosition.Y = RunnerCharacter->GetActorLocation().Y;

		ReferenceToSpawn->GetFloorPlane()->SetActorLocation(FloorPosition);
		...
		if (RunnerCharacter->GetActorLocation().Z <= FloorPosition.Z)
		{
			...
			else
			{
				...
				GetWorld()->GetTimerManager().SetTimer(RespawnTimerHandle, this, &ARoundGameMode::OnRespawn, 1.0f, false);
			}
		}
	}
...
EPlatformType ARoundGameMode::GetPlatformTypeToSpawn(const EPlatformType& PlatformType)
{
...
void ARoundGameMode::SpawnNewPlatform()
{
...
TSubclassOf<class AModule> ARoundGameMode::GetModuleByType(const EPlatformType& PreviousType)
{
	for (int i = 0; i < PlatformTable.Num(); i++)
	{
...
void ARoundGameMode::OnRespawn()
{
	...
	if (GameState == EGameState::VE_PrepareGame)
	{
		if (PlayerController->GetUI() != nullptr)
		{
			...
			GetWorld()->GetTimerManager().SetTimer(RespawnTimerHandle, this, &ARoundGameMode::OnRespawn, 3.0f, false);
		}
	}
